...
const stripSpacesBefore = (node, spaces) => fixer => fixer.removeRange([node.range[0] - spaces, node.range[0]]);

exports.stripSpacesBefore = stripSpacesBefore;

const stripSpacesAfter = (node, spaces) => fixer => fixer.removeRange([node.range[1], node.range[1] + spaces]);

exports.stripSpacesAfter = stripSpacesAfter;

const addSpaceBefore = node => fixer => fixer.insertTextBefore(node, ' ');

exports.addSpaceBefore = addSpaceBefore;

const addSpaceAfter = node => fixer => fixer.insertTextAfter(node, ' ');

exports.addSpaceAfter = addSpaceAfter;

const replaceWithSpaceBefore = (node, spaces) => fixer => fixer.replaceTextRange([node.range[0] - spaces, node.range[0]], ' ');

exports.replaceWithSpaceBefore = replaceWithSpaceBefore;

const replaceWithSpaceAfter = (node, spaces) => fixer => fixer.replaceTextRange([node.range[1], node.range[1] + spaces], ' ');
