...
const isIdentifier = (node, name) => node && node.type === 'Identifier' && node.name.match(name);
...
const create = context => {
  ...
  return {
    ...
    Program() {
      for (const comment of context.getSourceCode().getAllComments().filter(node => node.type === 'Block' || node.type === 'Line')) {
