...
const create = context => {
  ...
  return {
    Program(node) {
      ...
      const potentialFlowFileAnnotation = _lodash.default.find(context.getSourceCode().getAllComments(), comment => looksLikeFlowFileAnnotation(comment.value));
      ...
      if (potentialFlowFileAnnotation) {
        ...
        if (!never && !nextLineIsEmpty) {
          context.report({
            fix: fixer => fixer.insertTextAfter(potentialFlowFileAnnotation, newline),
